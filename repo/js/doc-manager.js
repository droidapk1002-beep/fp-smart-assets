...
// sont pas comptables côté navigateur (téléchargement bloqué) : pages vaut 0. Sinon : HEAD
...
async function detectPdfMeta(url) {
  ...
  if (gdMatch) {
    try {
      const res = await fetch('https://drive.google.com/file/d/' + gdMatch[1] + '/preview', { signal: AbortSignal.timeout(20000) });
      if (res.ok) {
        const bytes = extractDriveSizeFromViewerHtml(await res.text());
        if (bytes) return { size: formatFileSize(bytes), pages: 0 };
      }
    } catch {}
    return null;
  }
