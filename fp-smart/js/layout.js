...
function wireLayoutEvents() {
  ...
  if (modal && modalCloseBtn) {
    ...
    document.addEventListener('keydown', function(e) {
      if (e.key === 'Escape' && modal.classList.contains('open')) {
        ...
        if (modalSearchInp && modalSearchInp.style.display !== 'none') {
          modalSearchInp.style.display = 'none';
          modalSearchInp.value = '';
          var modalMsgs = document.getElementById('modal-ai-messages');
          if (modalMsgs) modalMsgs.querySelectorAll('.chat-msg').forEach(function(m) { m.style.display = ''; });
          return;
        }
  ...
  document.addEventListener('keydown', function(e) {
    if (e.key === '/' && !e.ctrlKey && !e.metaKey) {
      ...
      var dm = document.getElementById('doc-modal');
      if (dm && dm.classList.contains('open')) {
        var msi = document.getElementById('modal-ai-search-inp');
        var mst = document.getElementById('modal-ai-search-toggle');
        if (msi && mst) {
          if (msi.style.display === 'none') { msi.style.display = ''; }
          msi.focus();
          e.preventDefault();
        }
        return;
...
function openDocModal(docId) {
  ...
  details.innerHTML = '' +
    ...
    '<div><dt>' + (t('modal.year') || 'Ann\u00e9e') + '</dt><dd class="ltr-only">' + escapeHTML(String(doc.year || '&mdash;')) + '</dd></div>' +
