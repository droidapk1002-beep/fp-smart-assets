...
function wireTableEvents(tbody) {
  ...
  if (bulkEditBtn && bulkPanel) {
    ...
    if (bulkHost) {
      var bhList = document.getElementById('bulk-datalist-host');
      if (bhList) bhList.innerHTML = (APP.db.hosts || []).map(h => '<option value="' + escapeAttr(localized(h.name)) + '">').join('');
    }
    var bsList = document.getElementById('bulk-datalist-url-source');
    if (bsList) { var srcSet = {}; (APP.db.documents || []).forEach(d => { if (d.urlSource) srcSet[d.urlSource] = true; }); bsList.innerHTML = Object.keys(srcSet).map(u => '<option value="' + escapeAttr(u) + '">').join(''); }
    var bpList = document.getElementById('bulk-datalist-url-pdf');
    if (bpList) { var pdfSet = {}; (APP.db.documents || []).forEach(d => { if (d.urlPdf) pdfSet[d.urlPdf] = true; }); bpList.innerHTML = Object.keys(pdfSet).map(u => '<option value="' + escapeAttr(u) + '">').join(''); }
...
function wireDocManagerEvents() {
  ...
  descFr.addEventListener('input', () => {
    ...
    descTimer = setTimeout(async () => {
      if (descAutoFill && val) descAr.value = await translateText(val, 'ar');
      if (descAutoFill && val) descEn.value = await translateText(val, 'en');
    }, 600);
  });

  descAr.addEventListener('input', () => { descAutoFill = false; });
  descEn.addEventListener('input', () => { descAutoFill = false; });
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (editingIsPending) {
      ...
      scheduleAutoPush();
      applyLocalDocOverrides();
      if (!editingDocId && typeof addRecentDoc === 'function') addRecentDoc(doc.id);
    }
    renderAdminDocTable();
    resetDocForm();
...
function _toB64(str) { return btoa(unescape(encodeURIComponent(str))); }
function _fromB64(b) { return decodeURIComponent(escape(atob(b))); }
function encryptPAT(pat) {
  if (!pat) return '';
  var key = 'fp-salt-x9k';
  var enc = _xorEncode(pat, key);
  return 'ENC:' + _toB64(enc);
}
...
async function buildExportObj() {
  ...
  var encryptedPAT = encryptPAT(ghCfg.pat || '');
  return {
    meta: APP.db.meta,
    ...
    hidden: {
      docIds: (function() { try { return JSON.parse(localStorage.getItem('fp_hidden_doc_ids') || '[]'); } catch { return []; } })(),
      diplomaIds: (function() { try { return JSON.parse(localStorage.getItem('fp_hidden_diploma_ids') || '[]'); } catch { return []; } })(),
      semesterIds: (function() { try { return JSON.parse(localStorage.getItem('fp_hidden_semester_ids') || '[]'); } catch { return []; } })(),
      moduleIds: (function() { try { return JSON.parse(localStorage.getItem('fp_hidden_module_ids') || '[]'); } catch { return []; } })(),
      specialtyIds: (function() { try { return JSON.parse(localStorage.getItem('fp_hidden_specialty_ids') || '[]'); } catch { return []; } })()
    },
    githubSync: {
      repo: ghCfg.repo || '',
...
/* ---- GitHub Sync ---- */
var GH_CONFIG_KEY = 'fp_github_sync';
var GH_AUTO_PUSH_KEY = 'fp_github_auto_push';
...
function saveGithubConfig(cfg) { try { localStorage.setItem(GH_CONFIG_KEY, JSON.stringify(cfg)); } catch {} }
function getGithubAutoPush() { return localStorage.getItem(GH_AUTO_PUSH_KEY) === 'true'; }
function setGithubAutoPush(val) { try { localStorage.setItem(GH_AUTO_PUSH_KEY, val ? 'true' : 'false'); } catch {} }
...
async function pushDbToGithub() {
  ...
  try {
    ...
    var body = { message: 'chore(db): update db.json via admin [' + new Date().toISOString().slice(0, 19) + ']', content: encodedContent, branch: cfg.branch };
...
function importDbData(data) {
  try {
    ...
    localStorage.removeItem('fp_deleted_doc_ids');
