...
function getSelectedKey() {
  return localStorage.getItem('fp_selected_key_idx');
}
...
function renderKeys() {
  ...
  container.innerHTML = keys.map((k, i) => `
    <div class="key-row" data-idx="${i}">
      <div class="key-info">
        <strong>${escapeHTML(k.provider)}</strong>
        <code>${escapeHTML(k.apiKey.slice(0, 15))}…${escapeHTML(k.apiKey.slice(-4))}</code>
        <span>${escapeHTML(k.model)}</span>
      </div>
...
function generateManifestJSON() {
  ...
  return { items: items, generatedAt: new Date().toISOString(), count: items.length, source: 'fp-smart-no-backend' };
}
...
function downloadManifestJSON() {
  ...
  a.href = url;
  a.download = 'manifest-fp-smart-no-backend-' + Date.now() + '.json';
